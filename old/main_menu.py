...
def set_new_game(master,e_list,err_msg):
    ...
    # has the value in range [10,40]
    try:
        game_init_data["n"] = int(e1_text)
        if (game_init_data["n"] <= 40) and (game_init_data["n"]>=10):
            n_check = True
        else:
            n_check = False
    except:
    ...
    # has value in range [10,25]
    try:    
        game_init_data["m"] = int(e2_text)
        if (game_init_data["m"] <= 25) and (game_init_data["m"]>=10):
            m_check = True
        else:
            m_check = False
    except:
...
def MainMenu(msg):
    ...
    # max value labels
    l11 = tk.Label(data_frame, text="[10,40] int", anchor="w")
    l11.place(x=padx+gap+obj_width+text_box_width,
              y=pady,
              width=text_box_width,
              height=obj_height)
    l12 = tk.Label(data_frame, text="[10,25] int", anchor="w")
