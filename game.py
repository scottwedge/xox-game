...
class Game:
	...
	def game_reset(self,pID):
		# who won goes first in next round
		if self.won[pID] and self.finish:
	...
	# def dummy_check(self,pID,rect_pos):
	# 	xpos = int((rect_pos.centerx-10-self.block_width/2)/self.block_width)
	# 	ypos = int((rect_pos.centery-10-self.block_width/2)/self.block_width)

	# 	# left-right
	# 	# check if  the begin / end is valid
	# 	# if not given the boundary value (0 or max)
	# 	xstart = xpos-(self.max_len-1)
	# 	xend = xpos+(self.max_len+1)+1
	# 	ystart, yend = ypos,ypos+1
	# 	aux = (self.matrix[xstart:xend,ystart:yend]).flatten()
	# 	for i_ in range(self.max_len):
	# 		suma = np.sum(aux[i_:self.max_len+i_])
	# 		if suma==(pID*self.max_len):
	# 			self.ready = False
	# 			self.finish = True
	# 			self.won[pID] = True
	# 			self.score[pID] += 1
